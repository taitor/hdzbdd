...
//  printf("SB table: %d bytes (%1.2f bpc)\n",ns * sizeof(*sm) * 2, (double)ns * sizeof(*sm)*2 * 8/n);
  if (opt & OPT_MIN_POS) {
    mymalloc(smi, ns, 0);  b->idx_size += ns * sizeof(*smi);
//    printf("smi size %ld\n", ns * sizeof(*smi));
    b->smi = smi;
  } else {
    b->smi = NULL;
...
//    }
    ...
    if (i % SB == SB-1 || i==n-1) {
      ...
      if (opt & OPT_MIN_POS) smi[i/SB] = (sb_type)(min_pos - (i/SB*SB));
    }
...
i64 bp_construct2(bp *b,i64 n, pb *B, i64 opt, bpsub *e)
{
  ...
  mymalloc(b->da,1,0);
//  darray_construct(b->da,n,B, opt & OPT_FAST_PREORDER_SELECT);
  darray_construct2(b->da,n,B, opt & OPT_NO_RANK, e);
//  b->idx_size += b->da->idx_size;
//  printf("preorder rank/select table: %d bytes (%1.2f bpc)\n",
//         b->da->idx_size,(double)b->da->idx_size*8/n);
...
//  printf("MB table: %d bytes (%1.2f bpc)\n",(nm+m_ofs) * sizeof(*mm) * 2, (double)(nm+m_ofs) * sizeof(*mm)*2 * 8/n);
  ...
  for (i=0; i<n; i++) {
    i64 min_pos, max_pos;
    d += e->next(e->env)*2-1; // d = depth(b, i);
//    if (d != depth(b, i)) {
//      printf("i=%ld d=%ld depth=%ld\n", i, d, depth(b, i));
//    }
    if (i % SB == 0) {
    ...
    if (i % SB == SB-1 || i==n-1) {
      ds = ds2;
//      if (ds != depth(b,(i/SB)*SB-1)) {
//        printf("i=%ld ds=%ld depth=%ld\n", i, ds, depth(b,(i/SB)*SB-1));
//      }
      if (m - ds + SB < 0 || m - ds + SB >= (1<<(8*sizeof(sb_type)))) {
        printf("error m=%d ds=%d\n",m,ds);
      }

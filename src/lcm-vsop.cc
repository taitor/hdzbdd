...
/*************************************************************************/
CtoI LCM (PROBLEM *PP, int item, QUEUE *occ, WEIGHT frq, WEIGHT pfrq, int depth){ //ZDD 
  int ii, xx; //ZDD 
  CtoI F, G; //ZDD 
  F = CtoI(0); //ZDD
  ITEMSET *II = &PP->II;
...
// loop for recursive calls
  ...
  while ( QUEUE_LENGTH_ (PP->itemcand) > 0 ){
    ...
    if ( PP->occ_pw2[e] >= MAX(II->frq_lb, II->posi_lb) ){  // if the item is frequent
      ...
      roots[depth] = F.GetZBDD().GetID(); // F を保存
//      printf("test1 F size %ld\n", F.Size());
      G = LCM (PP, e, &TT->OQ[e], PP->occ_w2[e], PP->occ_pw2[e], depth+1); // recursive call //ZDD 
#ifdef USE_DENSE
      ZBDD h = ZBDD_ID(roots[depth]);
      F = CtoI(h); // 新しい F を復元
#endif
//      printf("test2 F size %ld\n", F.Size());
      F = CtoI_Union(F, G); //ZDD




#ifdef USE_DENSE // sada 2015/6/9
  {
      int i;
//      printf("F size %ld\n", F.Size());
      roots[depth] = F.GetZBDD().GetID();
      roots[depth+1] = bddnull;
#if 0
      printf("compress #roots %d\n", depth+1);
      for (i=0; i<depth+1; i++) {
        printf("%d:%ld size %ld\n", i, roots[i], bddsize(roots[i]));
      }
#endif
      compress(roots, opt);
#if 0
...
CtoI CtoI_Lcm2 () { //ZDD 
  ...
  { //ZDD 
    F = CtoI(0); //ZDD
    if ( !ERROR_MES ) G = LCM (&LCM_PP, LCM_PP.TT.T.clms, &LCM_PP.oo, LCM_PP.TT.total_w_org, LCM_PP.TT.total_pw_org, 0); //ZDD 
    else G = CtoI(0); //ZDD 
    F = CtoI_Union(F, G); //ZDD

...
typedef struct {
  i64 id;          // ブロックのID
  int num;         // ブロック内の要素数
//  i64 block[RRR*4];      // ブロック
  i64 block[RRR];      // ブロック
  int next;        // リストの次の要素
} block_cache;


int block_cache_max, block_cache_num, block_cache_head;
block_cache *block_cache_array = NULL;
i64 block_cache_hit, block_cache_miss;

void block_cache_init(int size)
{
  int i;
  if (block_cache_array) {
    printf("block_cache_init:  block_cache_array = %p\n", block_cache_array);
    exit(1);
  }
  block_cache_array = malloc(size * sizeof(*block_cache_array));
  block_cache_max = size;
  block_cache_num = 0;
  block_cache_head = -1;
  block_cache_hit = 0;
  block_cache_miss = 0;
...
{
  int i;
  if (block_cache_array==NULL) {
    printf("block_cache_free:  block_cache_array = %p\n", block_cache_array);
    exit(1);
  }
  free(block_cache_array);
  block_cache_array = NULL;
}
#endif
...
int darray_construct2(darray *da, i64 n, pb *buf, int opt, bpsub *e)
{
  i64 i,j,k,m;
  i64 nl;
  i64 p,pp;
...
//  w = getzerorun(B,p);
  w = 0;
  while (w < GAMMATBL && getbit(B,p+w)==0) w++;
...
static pb blocktmp[RRR/D];
static i64 blocktmp2[RRR*4];
...
pb tmpbuf[4];
  ...
  for (i1=0; i1<RRR/D; i1++) blocktmp[i1] = 0;
  ...
  for (loop = 1; loop <= 2; loop++) {
    ...
    if (loop == 1) {
      // fprintf(stderr, "original length = %ld, RLE = %ld\n", n, p1);
      mymalloc(compB, (p1+D-1)/D+1, 0);
      da->idx_size += ((p1+D-1)/D+1) * sizeof(pb);
      for (i1=0; i1<(p1+D-1)/D+1; i1++) compB[i1] = 0;
    }
  }
...
int darray_comp_rle2(darray *da, bpsub *e)
{
i64 n;
i64 i1, i2, i3;
int loop;
...
i64 darray_decode_block_rle2(darray *da, i64 s, i64 t, i64 **out2)
{
pb *buf;
i64 p, q, r;
i64 i;
...
//    printf("seach %ld:", ith);
    while (i >= 0) {
//      printf("(%d:%ld) ", i, leaf_cache_array[i].id);
      if (block_cache_array[i].id == blk) { // 見つかった
        num = block_cache_array[i].num;
        *out2 = block_cache_array[i].block;

        if (j != -1) { // 直前の要素がある
          block_cache_array[j].next = block_cache_array[i].next;
          block_cache_array[i].next = block_cache_head;
          block_cache_head = i; // i を先頭に
        }
//        printf(" found\n");
        block_cache_hit++;
        return num;
      ...
      j2 = j;
      j = i;
      i = block_cache_array[i].next;
    }
//    printf("\n");
    // 見つからなかったとき
    block_cache_miss++;
    if (block_cache_num == block_cache_max) { // キャッシュがいっぱい
      block_cache_array[j2].next = -1;
      block_cache_num--;
      block_cache_array[j].next = block_cache_head;
      block_cache_head = j; // 新しい先頭は j
      block_cache_array[block_cache_head].id = blk;
    } else {
      block_cache_array[block_cache_num].next = block_cache_head;
      block_cache_head = block_cache_num;
      block_cache_array[block_cache_head].id = blk;
    }
    *out2 = out = block_cache_array[block_cache_head].block;
  } else {
    *out2 = out = blocktmp2;
  }
...
#ifdef USE_CACHE
  if (block_cache_max > 0) {
    block_cache_array[block_cache_head].num = num;
    block_cache_num++;
  }

//  printf("cache %ld num = %ld\n", block_cache_head, num);
//  for (i=0; i<num; i++) printf("%ld ", out[i]);
//  printf("\n");
...
i64 darray_rank(darray *da, i64 i)
{
  ...
  if (da->opt & OPT_DARRAY_COMP_RLE) {
    ...
    s = (i>>logRRR)<<logRRR;
#if 1
    num = darray_decode_block_rle2(da, s, s+RRR-1, &blocktmp2);
    for (u = 0; u < num; u++) {
      x = blocktmp2[u];
      if (s + abs(x) > i) break;
      s += abs(x);
...
//////////////////////////////// *p は圧縮されていないデータへのポインタ
  if (da->opt & OPT_DARRAY_COMP_RLE) {
    i64 s;
    s = (i>>logRRR)<<logRRR;
    darray_decode_block_rle(da, s, s+RRR-1, blocktmp); // 実行されない
    p = &blocktmp[0];
...
//  if (da->opt & OPT_DARRAY_COMP_RLE) if (r != r2) {
//    printf("darrray_rank(%ld): r=%ld r2=%ld\n", i, r, r2);
//  }
...
//  if (da->opt & OPT_DARRAY_COMP_RLE) if (r != r2) {
//    printf("darrray_getbit(%ld): r=%ld r2=%ld\n", i, r, r2);
//  }

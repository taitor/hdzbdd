...
{
  if(v & 1)
    BDDerr("SOP::And1: VarID must be even number.", v);
...
SOP SOP::Support() const
{
  ...
  while(fz != 0)
  {
    t = fz.Top();
    fz = fz.OffSet(t);
//    if((t & 1)==0) f = f.FactorD(t).And1(t);
//    if (t&1) f -= SOP(1).And0(t+1);
    if (t&1) f = f.FactorD(t+1) + SOP(1).And0(t+1);
  }
  return f;
}
...
//  if(IsPolyCube() == 0) return 1;
  if(IsPolyCube() == 0) return *this;
  int top = Top();
  SOP f = *this;
//  cout << "Divisor: f " << endl;  f.PrintPla();  cout << endl;
  SOP g = Support();
//  cout << "Divisor: g " << endl;  g.PrintPla();  cout << endl;
  int t;
  while(g != 0)
  {
    t = g.Top();
...
//    cout << "Divisor: g " << endl;  g.PrintPla();  cout << endl;
    SOP f0 = f.Factor0(t);
    SOP f1 = f.Factor1(t);
//    cout << "Divisor: f0 " << endl;  f0.PrintPla();  cout << endl;
//    cout << "Divisor: f1 " << endl;  f1.PrintPla();  cout << endl;
